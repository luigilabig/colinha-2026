 ...
 * em public/data/. Feito para rodar no GitHub Actions, sem PC.
 *
 * Roda sozinho todo dia. Não precisa fazer nada depois de configurar.
 */
...
async function baixar() {
  ...
  const r = await request(URL, { maxRedirections: 5 });
  if (r.statusCode !== 200) {
    throw new Error(
      `TSE respondeu ${r.statusCode}.\n` +
      `Confira se o arquivo ainda existe: ${URL}\n` +
...
function lerCsvs(buf) {
  ...
  if (!csvs.length) throw new Error("nenhum CSV dentro do ZIP — estrutura mudou");
  console.log(`→ ${csvs.length} CSVs no pacote`);

  const linhas = [];
  let cabecalhoLogado = false;
  for (const e of csvs) {
...
/**
 * Um representante por (uf, cargo, partido) — o de melhor número.
 * Senado é exceção: 2026 elege DUAS vagas por estado, então guardamos
 * os dois melhores e o app mostra um em cada voto.
 */
...
(async () => {
  try {
    ...
    if (!cands.length) throw new Error("nenhuma candidatura válida — confira as colunas listadas acima");
    ...
    console.log("  amostra:", JSON.stringify(Object.values(ex)[0]).slice(0, 200));
  } catch (e) {
    console.error("\n✗ FALHOU:", e.message);
    process.exit(1);
  }
